...
//       } else {
//         toast.error(data?.message || "Échec du chargement des applications.");
//       }
//     } catch (error) {
//       console.error(error?.response?.data || "Applications de récupération d'erreurs");
//       toast.error(
//         error?.response?.data?.message || "Impossible de récupérer les candidatures"
//       );
//     } finally {
//       setIsLoading(false);
//     }
...
//       if (data?.success) {
//         toast.success(data?.message || "Statut mis à jour avec succès.");
...
//                         </a>
//                       ) : (
//                         <span className="text-xs text-gray-400">Pas de CV</span>
//                       )}
//                     </td>
//                     <td className="px-4 py-4 text-center">
...
const ViewApplications = () => {
  ...
  const fetchApplications = async () => {
    if (!companyToken) return; // ⚡ évite appel si pas de token
    ...
    try {
      const { data } = await axios.post(
        ...
        {
          headers: {
            Authorization: `Bearer ${companyToken}`, // ⚡ standard pour JWT
          },
        }
      );

      if (data?.success) {
        setApplications(data.viewApplicationData || []);
      } else {
        toast.error(data?.message || "Échec du chargement des applications.");
      }
    } catch (error) {
      console.error("Erreur récupération applications:", error?.response?.data);
      toast.error(
        error?.response?.data?.message || "Impossible de récupérer les candidatures"
      );
    } finally {
  ...
  const handleStatusUpdate = async (id, status) => {
    ...
    try {
      ...
      if (data?.success) {
        toast.success(data?.message || "Statut mis à jour avec succès.");
        await fetchApplications(); // Recharger les applications
      } else {
        toast.error(data?.message || "Échec de la mise à jour du statut");
      }
    } catch (error) {
  ...
  if (!applications || applications.length === 0) {
    return <div className="text-center py-10 text-gray-500">Aucune application trouvée.</div>;
  }
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[150px]">Utilisateur</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[180px]">Titre d'emploi</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">Emplacement</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden lg:table-cell">Date</th>
                <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Resumer</th>
                <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[180px]">Statut</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
              {applications.reverse().map((job, index) => (
                <tr key={job._id} className="hover:bg-gray-50 transition-colors duration-150">
                  ...
                  <td className="px-4 py-4">
                    <div className="flex items-center">
                      ...
                      />
                      <div className="ml-3 min-w-0">
                        <p className="text-sm font-medium text-gray-900 truncate">{job?.userId?.name || "Unknown"}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-4 text-sm text-gray-900 max-w-[180px] truncate">{job?.jobId?.title}</td>
                  <td className="px-4 py-4 text-sm text-gray-500 hidden md:table-cell">{job?.jobId?.location}</td>
                  <td className="px-4 py-4 text-sm text-gray-500 hidden lg:table-cell">{job?.date ? moment(job.date).format("ll") : "N/A"}</td>
                  <td className="px-4 py-4 text-center">
                    {job?.userId?.resume ? (
                      <a
                        href={job.userId.resume}
                      ...
                      >
                        Voir
                        <img src={assets.resume_download_icon} alt="" className="ml-1.5 h-3 w-3" />
                      </a>
                    ) : (
                      <span className="text-xs text-gray-400">Pas de CV</span>
                    )}
                  ...
                  <td className="px-4 py-4 text-center">
                    {updatingStatus === job._id ? (
                      ...
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${job.status === "Accepter" ? "text-green-800" : "text-red-800"}`}>
                        {job.status}
                      </span>
                    )}
                  </td>
