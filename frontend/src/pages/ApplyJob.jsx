  ...
  const applyJob = async (jobId) => {
    try {
      if (!userData) {
        ...
        return toast.error("Veuillez vous connecter pour postuler");
      }
      if (!userData?.resume) {
        navigate("/applications");
        return toast.error("Veuillez Télécharger votre CV");
      }
      ...
      toast.error(error?.response?.data?.message || "Quelaue Chose ne va pas");
    }
  };
      ...
      <section>
        <div className="flex flex-col lg:flex-row justify-between border border-blue-200 rounded-lg bg-blue-50 p-8 lg:p-12 mb-8">
          <div className="flex flex-col md:flex-row md:items-center gap-6">
            <div className="w-20 h-20 flex-shrink-0 bg-white border border-gray-200 rounded-lg flex items-center justify-center">
              <img
                ...
                alt={jobData?.companyId?.name || "Logo de l'entreprise"}
            ...
            <div className="flex-1 mb-6 md:mb-0">
              ...
              <div className="flex flex-wrap items-center gap-4 text-gray-600">
                <div className="flex items-center gap-1.5">
                  <img src={assets.suitcase_icon} alt="Entreprise" />
                  <span>{jobData?.companyId?.name || "Entreprise Inconnue"}</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <User size={20} />
                ...
                <div className="flex items-center gap-2">
                  <img src={assets.money_icon} alt="Salaire" />
                  <span>
                    CTC:{" "}
                    {jobData?.salary
          ...
          <div className="md:mt-6 flex flex-col items-start gap-2.5">
            ...
            >
              {alreadyApplied ? "Already Applied" : "Postulez dès maintenant"}
            </button>
            <div className="flex items-center gap-1.5 text-sm text-gray-600">
              <Clock size={18} />
              <span>Publié {moment(jobData?.date).fromNow()}</span>
            </div>
          </div>
        </div>
